...
router.post("/", async (req, res, next) => {
  ...
  let selectorderTbQuery =
    "SELECT ot.*, gt.Goods_name,gt.price FROM order_tb ot JOIN goods_tb gt ON ot.goods_no = gt.Goods_no WHERE user_id = ? ";
  ...
  try {
    ...
    await dba.select(selectorderTbQuery, selectorderTbParams).then((result) => {
      if (result.length == 0) return console.log("no order");
      console.log(result);
